...
function tiempoSinAtender(fechaEscalado) {
  if (!fechaEscalado) return '0s';
  const diff = Date.now() - new Date(fechaEscalado).getTime();
  const seg = Math.floor(diff / 1000);
  if (seg < 60) return `${seg}s`;
  return `${Math.floor(seg / 60)}m ${seg % 60}s`;
}

export default function PedidoEscalado({ pedido }) {
  const [cargando, setCargando] = useState(false);
  const [tiempoTexto, setTiempoTexto] = useState(
    tiempoSinAtender(pedido.escalado_operadora_at)
  );

  // Actualizar tiempo sin atender cada segundo
  useState(() => {
    const id = setInterval(() => {
      setTiempoTexto(tiempoSinAtender(pedido.escalado_operadora_at));
    }, 1000);
    return () => clearInterval(id);
  });

  const sinAtenderMinutos =
    pedido.escalado_operadora_at
      ? (Date.now() - new Date(pedido.escalado_operadora_at).getTime()) / 60000
      : 0;
  ...
  return (
    <div
      className={`bg-tarjeta rounded-xl border-l-4 border-buscando p-4 ${
    ...
    >
      ...
      <div className="flex items-start justify-between mb-2">
        <div>
          <span className="text-xs font-bold text-buscando">🚨 Escalado</span>
          <h3 className="text-sm font-bold text-white leading-tight">
            #{pedido.id} · {pedido.restaurante || 'Sin restaurante'}
          </h3>
        </div>
        <div className="text-right">
          <span className="text-xs text-gray-400">{tiempoTexto}</span>
          {sinAtenderMinutos >= 2 && (
            <div className="text-[10px] text-buscando font-bold mt-0.5 animate-pulse">
              ⚠️ Sin atender
            </div>
          )}
        </div>

// Selector cuando el restaurante tiene varias sucursales registradas: la
// operadora elige de cuál retira el moto. La primera viene ordenada por
// distancia al cliente (useSucursales) y se destaca como recomendada.
export default function SelectorSucursal({ sucursales, sucursalSeleccionada, onSeleccionar }) {
  if (sucursales.length <= 1) return null;
  const recomendada = sucursales[0]?._distancia != null ? sucursales[0] : null;
  return (
    <div className="bg-yellow-900/30 border border-yellow-600/50 rounded-lg p-2 mb-2">
      ...
      {recomendada && (
        <div className="bg-green-900/40 border border-green-600/50 rounded-lg p-1.5 mb-1.5">
          <p className="text-[9px] text-green-400 font-bold mb-0.5">✅ MÁS CERCANA AL CLIENTE</p>
          <p className="text-[11px] text-white font-semibold">
            {recomendada.sucursal_nombre} — {recomendada._distancia.toFixed(1)} km
          </p>
          <p className="text-[9px] text-gray-400">{recomendada.direccion}</p>
        </div>
      )}
      <select
        ...
        onChange={(e) => {
          const s = sucursales.find((x) => x.id === Number(e.target.value));
          onSeleccionar(s || null);
        }}
      >

...
const URL_PANEL_WEB = 'https://dewansas.com/restaurante-hp/web/';
...
function cargarPanel() {
  if (!mainWindow || mainWindow.isDestroyed()) return;
  if (isDev) {
    mainWindow.loadURL('http://localhost:5174/restaurante-hp/');
    return;
  }
...
ipcMain.handle('notify-order', (_event, { title, body }) => {
  if (Notification.isSupported()) {
    const n = new Notification({
      ...
      body: body || 'Tienes un nuevo pedido en Happy Pollo',
      urgency: 'critical',
    });
    n.show();
    n.on('click', () => {
